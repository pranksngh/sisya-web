...
<Box
  ...
  ></Box>
      ...
      {/* Right Panel */}
      <Box className="right-panel" sx={{ flex: 3, display: 'flex', flexDirection: 'column', padding: 2 }}>
        <video
        ...
        {isUserListVisible ? (
          ...
          >
            ...
            >
              {messages.length === 0 ? (
                <Box className="no-chats" sx={{ textAlign: 'center', color: '#757575' }}>
    ...
    <Box
      className="footer"
      sx={{ display: 'flex', justifyContent: 'space-around', padding: 2, backgroundColor: '#ffffff', borderTop: '1px solid #e0e0e0' }}
    >
      <IconButton color={isMuted ? 'secondary' : 'primary'} onClick={toggleMute}>
        {isMuted ? <MicOff /> : <Mic />}
      </IconButton>
      <IconButton color={!isCameraEnabled ? 'secondary' : 'primary'} onClick={toggleCamera}>
        {isCameraEnabled ? <Videocam /> : <VideocamOff />}
      </IconButton>
      <IconButton color={isScreenShared ? 'primary' : 'default'} onClick={isScreenShared ? stopScreenShare : startScreenShare}>
        <DesktopWindows />
      </IconButton>
