...
export default function VideoCallPage() {
    ...
    const initZego = async () => {
      try {
        ...
        zg.on("roomStreamUpdate", async (roomID, updateType, streamList) => {
          console.log(
            "Room stream update type is " + JSON.stringify(updateType)
          );
    ...
    if (zegoEngine && message.trim() !== "") {
      zegoEngine
        ...
        .then(() => {
          setMessages([
            ...messages,
            { userID: "prashant90654", userName, message },
          ]);
          setMessage("");
  ...
  const toggleUserMic = (userID) => {
    ...
    showAlert(
      `User ${userID} microphone ${newMicStatus ? "muted" : "unmuted"}`,
      "info"
    );
  };
        ...
        >
          ...
          <IconButton
            ...
            sx={{
              ...
              color: "#fff",
              "&:hover": {
                backgroundColor: !isCameraEnabled ? "#d32f2f" : "#1565c0",
              },
            }}
          >
            {isCameraEnabled ? <FaVideo /> : <FaVideoSlash />}
