...
const PilotRegistrationPaidData = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (result.success) {
        const successLeads = result.leads
          .filter((lead) => lead.status === "success")
          .sort((a, b) => new Date(b.createdOn) - new Date(a.createdOn)); // Sort newest first
        setLeads(successLeads);
      }
    } catch (error) {
  ...
  return (
    <Paper elevation={0} sx={{ padding: 2 }}>
      <Typography variant="h6" sx={{ mb: 2, fontWeight: "bold" }}>
        Paid Pilot Registrations (Success Only)
      </Typography>
...
export default PilotRegistrationPaidData;
