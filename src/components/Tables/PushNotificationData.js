...
const PushNotificationData = () => {
  ...
  const [selectedStudents, setSelectedStudents] = useState([]);
  const [selectedClass, setSelectedClass] = useState("");
  const [sendTo, setSendTo] = useState("class"); // "class" or "students"
  const [isSubmitting, setIsSubmitting] = useState(false);
  ...
  const sendNotification = async (tokens, base64Image) => {
    if (sendTo === "class" && selectedClass) {
      ...
      tokens.push(
        ...studentsInClass.map((student) => student.deviceId).filter(Boolean)
      );
    } else if (sendTo === "students" && selectedStudents.length > 0) {
      // Send to specific selected students by phone number
      tokens.push(
        ...selectedStudents
          .map(
            (studentPhone) =>
              studentList.find((s) => s.phone === studentPhone)?.deviceId
          )
          .filter(Boolean)
      );
    } else {
  ...
  return (
    <Box p={3} maxWidth="600px" mx="auto">
      ...
      <form onSubmit={handleSend}>
        ...
        {/* Image Upload section - Uncomment if needed
        <Box mb={2}>
          <Typography variant="body1" gutterBottom>
            Upload Image
          </Typography>
        ...
        */}
        <Box mb={2}>
          <FormControl component="fieldset">
            <FormLabel component="legend">Send Notification To</FormLabel>
            <RadioGroup
              row
              value={sendTo}
